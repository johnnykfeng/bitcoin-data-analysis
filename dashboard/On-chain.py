...
cities = {
    k: v for k, v in sorted(cities.items(), key=lambda item: item[1], reverse=True)
}
del cities[None]
...
def load_heavy_data():
    ...
    second_to_last_block = requests.get(
        f'https://blockchain.info/block-height/{lastblock["height"]-1}?format=json'
    ).json()
...
addr_df = addr_df.loc[
    (addr_df["Date"] >= pd.Timestamp(start_date))
    & (addr_df["Date"] <= pd.Timestamp(end_date))
]
tx_df = tx_df.loc[
    (tx_df["Date"] >= pd.Timestamp(start_date))
    & (tx_df["Date"] <= pd.Timestamp(end_date))
]
hash_df = hash_df.loc[
    (hash_df["Date"] >= pd.Timestamp(start_date))
    & (hash_df["Date"] <= pd.Timestamp(end_date))
]
...
with col2:
    ...
    col2.metric(
        label="Hash rate attuale",
        value=f"{current_hash} TH/s",
        delta=f"{delta} TH/s rispetto a 3 giorni fa",
    )
    st.divider()
    ...
    last_block_minimg_time = datetime.fromtimestamp(
        lastblock["time"]
    ) - datetime.fromtimestamp(second_to_last_block["blocks"][0]["time"])
